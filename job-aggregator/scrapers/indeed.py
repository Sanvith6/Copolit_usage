"""
Indeed job scraper using Apify Actor.
...
def scrape_indeed_jobs(
    ...
    """
    Run the Apify Indeed jobs scraper and return a list of raw job dicts.
    ...
    if not APIFY_API_TOKEN:
        logger.warning("APIFY_API_TOKEN not set — returning empty result for Indeed scraper.")
        return []

    run_input = _build_run_input(keywords, location, max_jobs)
    run_url = f"{APIFY_BASE_URL}/acts/{ACTOR_ID}/run-sync-get-dataset-items"

    logger.info("Starting Indeed Apify scraper for keywords: %s", keywords)
    try:
        ...
        logger.error("Indeed scraper HTTP error: %s", exc)
        return []
    ...
    logger.info("Indeed scraper returned %d jobs.", len(jobs))
    return jobs
